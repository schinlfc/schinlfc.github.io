...
Exit codes:
    ...
    2 — internal error (unreadable directory)

Surfaces that don't exist in this repo (e.g. the upstream Quarto guide,
intentionally omitted here) are skipped, not treated as errors.
"""
...
def main() -> int:
    ...
    # Only check surfaces that exist. Several listed surfaces (the Quarto
    # guide and its rendered `docs/` HTML) ship with the upstream template
    # but are intentionally absent in this content repo — see CLAUDE.md,
    # "Things this template intentionally omits". A missing surface simply
    # contributes no assertions; it is not an error.
    present = [p for p in SURFACES if p.exists()]
    skipped = [p for p in SURFACES if not p.exists()]
    if skipped:
        print("Skipped (not present in this repo): "
              + ", ".join(str(rel(p)) for p in skipped))
        print()
